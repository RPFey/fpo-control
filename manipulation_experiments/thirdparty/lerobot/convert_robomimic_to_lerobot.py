...
Usage:
  python convert_robomimic_to_lerobot.py --dataset /path/to/robomimic_dataset.hdf5 --output_dir /path/to/lerobot_dataset --repo_id [your-hf-account]/dataset-name
"""
...
def get_dataset_trajectories(
    dataset_path: str, filter_key: str | None = None, max_episodes: int | None = None
) -> list[str]:
    """
...
def get_action_names(env_name: str, action_dim: int) -> list[str]:
    ...
    if is_dexterous and action_dim == 24:
        ...
        return [
            ...
            "right_wrist_delta_pos_z",  # 0-2
            "right_wrist_delta_rot_rx",
            "right_wrist_delta_rot_ry",
            "right_wrist_delta_rot_rz",  # 3-5
            "right_thumb_flexion",
            "right_thumb_opposition",
            "right_index_flexion",  # 6-8
            "right_middle_flexion",
            "right_ring_flexion",
            "right_pinky_flexion",  # 9-11
            "left_wrist_delta_pos_x",
            "left_wrist_delta_pos_y",
            "left_wrist_delta_pos_z",  # 12-14
            "left_wrist_delta_rot_rx",
            "left_wrist_delta_rot_ry",
            "left_wrist_delta_rot_rz",  # 15-17
            "left_thumb_flexion",
            "left_thumb_opposition",
            "left_index_flexion",  # 18-20
            "left_middle_flexion",
            "left_ring_flexion",
            "left_pinky_flexion",  # 21-23
        ]
    elif is_humanoid and action_dim == 24:
        ...
        return [
            "robot0_delta_pos_x",
            "robot0_delta_pos_y",
            "robot0_delta_pos_z",  # Right arm position
            "robot0_delta_rot_rx",
            "robot0_delta_rot_ry",
            "robot0_delta_rot_rz",  # Right arm rotation
            "robot0_gripper",  # Right gripper
            "robot1_delta_pos_x",
            "robot1_delta_pos_y",
            "robot1_delta_pos_z",  # Left arm position
            "robot1_delta_rot_rx",
            "robot1_delta_rot_ry",
            "robot1_delta_rot_rz",  # Left arm rotation
            "robot1_gripper",  # Left gripper
        ]
    else:
        # Fallback to generic names if we don't recognize the configuration
...
def convert_robomimic_to_lerobot(
    ...
    # Get trajectory list
    demo_keys = get_dataset_trajectories(dataset_path, filter_key, max_episodes)
    ...
    with h5py.File(dataset_path, "r") as f:
        for episode_idx, demo_key in enumerate(tqdm(demo_keys, desc="Processing trajectories")):
