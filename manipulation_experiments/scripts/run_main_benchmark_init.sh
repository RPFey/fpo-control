    ...
    if [[ "$fpop_key" == "$task_key" ]]; then
      for SEED in 42 ; do
        run_cmd torchrun --nproc_per_node="$NUM_GPUS" finetune_online_rl.py \
          --base_policy_local_path /home/leiboshu/ActiveLearning/fpo-control/manipulation_experiments/runs/Fig_a7_basepolicy_checkpoints/square/step_25800/policy \
          --wandb_entity leiboshu \
          --distributed True \
          --base-policy-wandb-project flow-bc \
          --load-ema True    \
          --experiment "finetune-fpo++-square-init-${SEED}" \
          --wandb_project flow-bc-fpo-finetuning \
